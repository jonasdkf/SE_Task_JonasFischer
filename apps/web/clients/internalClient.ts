import { CustomerConsentCollectedFrom } from '@enterprise-commerce/core/platform/shopify/types/admin/admin.types';
...
const registerUser = async (input: PlatformUserCreateInput): Promise<Pick<PlatformUser, "id"> | undefined | null> => {
  ...
  try{
    //POST request to backend /register
  const { data } = await axios.post('http://localhost:3001/register', {email: input.email, password: input.password});

    //return the User data that was received from the response
    return data;
  } catch (error) {
